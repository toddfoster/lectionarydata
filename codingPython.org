...
- =he/hymn= should allow link to file, or inner text, else be a rubric allowing a hymn
- procedure: implement needed shortcodes for:
    o Lent3 (## The Word of God, ## The Holy Communion, ### Headings)
    o Good Friday (with special =he/propercollect=)
    o Palm Sunday
    o Easter Vigil
    o Everything else
    o Modify contents pages to automatically provide next & previous service
...
#+BEGIN_SRC python :python python3 :results output
  ...
  with open(srcfile, "r") as f:
      ...
      for row in reader:
          ...
          code = row[codeColumn].strip()
          date = datetime.datetime.strptime(row[dateColumn], '%m/%d/%y')
          liturgyDate = date.strftime(dateOutputFormat)
          publishDate = (date + datetime.timedelta(days=-6)).strftime(dateOutputFormat)
          filename = 'he-{0:%Y%m%d}-{1}.md'.format(date, code)
          if (code == "goodfriday" or code == "holysaturday"):
              filename = '{0}-{1:%Y%m%d}.md'.format(code, date)
          with open (targetdir + "/" + filename, "w") as t:
              ...
              for i in range(prepareColumn + 1, postscriptColumn):
                  if (i == psalmStyleColumn):
                      t.write('{{% he/propercollect %}}\n')
                      t.write('{{% he/lesson firstReading %}}\n')
                      t.write(shortcode(psalmStyleColumn))
                      t.write('{{% he/lesson secondReading %}}\n')
                      t.write('{{% he/gospel %}}\n')
                  else:
                      t.write(shortcode(i))
          count+=1
