...
Establish whether there are collisions to worry about in the processed textfilenames
Count duplicate references (for curiosity's sake)
Investigate references with mis-matched div's; repair
#+BEGIN_SRC python :python python3 :results output
          ...
          for i in range(len(citations)):
              ...
              for t in d.contents:
                  ...
                  if (not t.name in tags):
                       tags.append(t.name)
                  if (t.has_attr('class')):
                      if (not t.attrs['class'] in classes):
                          classes.append(t.attrs['class'])
...
Duplicates = 235
Tags: ['p', 'br', 'blockquote', 'span']
Classes: [['poetryText'], ['psalmText'], ['lessonText'], ['poetryIndent'], ['”lessonText”'], ['lessonIndent'], ['“lessonText“'], ['initCap'], ['lordSmallCaps'], ['collectText']]
#+end_example


Output html for use in sharedprayers
TODO: Psalms: retain latin title
TODO: Psalms: format for responsive reading
#+BEGIN_SRC python :python python3 :results output
  import os, urllib.request, urllib.parse, re, shutil, time, subprocess
  from bs4 import BeautifulSoup
  import bs4
              ...
              with open (destmark + filename + ".md", "w") as t:
                  ...
                  for e in d.contents:
                      ...
                      if (type(e) != bs4.element.Tag):
                          ...
                          continue
                      line = ""
                      # TODO: lordSmallCaps tags to LORD or GOD
                      # TODO: <BR> in the middle of a <p> to \n
                      if e.has_attr('class'):
                          newClass = e.attrs['class']
                          if newClass != lastClass:
                              if lastClass:
                                   line = "\n"
                              lastClass = newClass
                          if "poetry" in newClass or "psalm" in newClass or "ndent" in newClass:
                              line = line + "> "
                      line = line + e.getText() + "\n"
                      t.write(line)
